...
export default function AdminLocationsPage() {
  ...
  useEffect(() => {
    if (isAuthorized) fetchLocations()
  }, [isAuthorized])
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json()
        throw new Error(Object.values(data.errors ?? {}).join(', ') || 'Failed to add location')
      }
      setCity('')
      setCountry('')
      fetchLocations()
  ...
  return (
    <div className="min-h-screen bg-surface-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-surface-900">Locations</h1>
          <p className="text-surface-600 mt-1">Add or remove available service locations</p>
        </div>

        {/* Add form */}
        <form onSubmit={handleAdd} className="mb-8 rounded-[var(--radius-card)] border border-surface-200 bg-white p-6">
          <h2 className="text-lg font-semibold text-surface-900 mb-4">Add Location</h2>
          <div className="flex gap-3 flex-wrap">
            <input
              ...
              required
              className="flex-1 min-w-[140px] px-4 py-2 rounded-[var(--radius-input)] border border-surface-300 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
            />
            <input
              type="text"
              placeholder="Country"
        ...
        {isLoading ? (
          ...
          <p className="text-surface-500 text-sm">No locations yet.</p>
        ) : (
          <div className="rounded-[var(--radius-card)] border border-surface-200 bg-white overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-surface-50 border-b border-surface-200">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-surface-600">City</th>
                  <th className="px-4 py-3 text-left font-medium text-surface-600">Country</th>
              ...
              <tbody className="divide-y divide-surface-100">
                {locations.map(loc => (
                  <tr key={loc.id} className="hover:bg-surface-50">
                    <td className="px-4 py-3 text-surface-900">{loc.city ?? loc.name}</td>
                    <td className="px-4 py-3 text-surface-600">{loc.country ?? '—'}</td>
