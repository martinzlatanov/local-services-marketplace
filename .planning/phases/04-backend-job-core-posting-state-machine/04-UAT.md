...
number: 11
name: State Machine Transitions Complete Flow
expected: |
  Full flow: Create job (PENDING, v1) → Accept (ACCEPTED, v2) → Start (IN_PROGRESS, v3) → Complete (COMPLETED, v4).
  Each PATCH with correct version succeeds. Invalid version (e.g., v1 on second transition) returns 409.
awaiting: complete
